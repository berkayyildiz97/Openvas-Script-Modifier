...
if(get_kb_item("MS/ConfigMgr/Version"))
{
  ...
  if(path && "Could not find the install Location" >!< path)
  {
    ...
    if(confVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:confVer, fixed_version:"4.0.6487.2216", install_path:path);
...
if(get_kb_item("MS/SMS/Version"))
{
  ...
  if(path && "Could not find the install Location" >!< path)
  {
    ...
    if(confVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:confVer, fixed_version:"4.0.6487.22162.50.4253.3129", install_path:path);
        security_message(data:report);
        exit(0);
      }
    }
