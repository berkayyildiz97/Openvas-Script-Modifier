...
if(shareVer =~ "^12\..*")
{
  ...
  if(path)
  {
    ...
    if(dllVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:dllVer, vulnerable_range:"12.0 - 12.0.6721.4999", install_path:path);
...
if(shareVer =~ "^14\..*")
{
  ...
  if(path)
  {
    ...
    if(dllVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:dllVer, vulnerable_range:"14.0 - 14.0.7149.4999", install_path:path);
        security_message(port: 0, data: report);
        exit(0);
      }
    }
...
if(shareVer =~ "^15\..*")
{
  ...
  if(path)
  {
    ...
    if(dllVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:dllVer, vulnerable_range:"15.0 - 15.0.4719.999", install_path:path);
        security_message(port: 0, data: report);
        exit(0);
      }
    }
