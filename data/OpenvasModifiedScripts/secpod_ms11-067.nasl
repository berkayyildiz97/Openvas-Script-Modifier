    ...
    if(studioPath){
      ...
      if(sysVer && sysVer =~ "^8\.")
      {
        if(version_in_range(version:sysVer, test_version:"8.0", test_version2:"8.0.50727.5676")){
          report = report_fixed_ver(installed_version:sysVer, fixed_version:"8.0.50727.5677", install_path:studioPath);
...
foreach item (registry_enum_keys(key:key))
{
  ...
  if("\Microsoft.NET\Framework" >< path)
  {
    ...
    if(sysVer && sysVer =~ "^8\.")
    {
      ...
      {
        report = report_fixed_ver(installed_version:sysVer, fixed_version:"8.0.50727.56778.0.50727.5677", install_path:studioPath);
        security_message(data:report);
        exit(0);
      }
    }
