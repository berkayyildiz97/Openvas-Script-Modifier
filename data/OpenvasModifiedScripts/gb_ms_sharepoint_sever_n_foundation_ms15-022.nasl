...
if(shareVer =~ "^14\..*")
{
  ...
  if(dllVer)
  {
    ...
    {
      report = report_fixed_ver(installed_version:dllVer, fixed_version:"14.0.7145.5000", install_path:path);
...
if(shareVer =~ "^15\..*")
{
  ...
  if(path)
  {
    ...
    if(dllVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:dllVer, fixed_version:"14.0.7145.500015.0.4701.1000", install_path:path);
        security_message(data:report);
        exit(0);
      }
    }
