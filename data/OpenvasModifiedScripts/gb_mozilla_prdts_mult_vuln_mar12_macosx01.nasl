...
if(!isnull(ffVer))
{
  ...
  {
    report = report_fixed_ver(installed_version:ffVer, vulnerable_range:"4.0" + " - " + "10.0.2");
...
if(!isnull(seaVer))
{
  ...
  {
    report = report_fixed_ver(installed_version:seaVer, fixed_version:"2.8");
    security_message(port: 0, data: report);
  }
}
...
if(!isnull(tbVer))
{
  if(version_in_range(version:tbVer, test_version:"5.0", test_version2:"10.0.2")){
    report = report_fixed_ver(installed_version:tbVer, vulnerable_range:"5.0" + " - " + "10.0.2");
    security_message(port: 0, data: report);
  }
}
