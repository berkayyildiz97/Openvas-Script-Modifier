...
if(cmpPckVer && cmpPckVer =~ "^12\.")
{
  ...
  if(xlcnvVer && xlcnvVer =~ "^12\.")
  {
    ...
    {
      report = report_fixed_ver(installed_version:xlcnvVer, fixed_version:"12.0.6715.5000");
...
if(wordcnvVer && wordcnvVer =~ "^12\.")
{
  ...
  if(path)
  {
    ...
    if(sysVer && sysVer =~ "^12\.")
    {
      ...
      {
        report = report_fixed_ver(installed_version:sysVer, fixed_version:"12.0.6715.500012.0.6715.5000", install_path:path);
        security_message(data:report);
        exit(0);
      }
    }
