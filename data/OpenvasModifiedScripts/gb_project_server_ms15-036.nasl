...
if(psVer =~ "^14\..*")
{
  ...
  if(path)
  {
    ...
    if(dllVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:dllVer, fixed_version:"14.0.7141.5000", install_path:path);
...
if(psVer =~ "^15\..*")
{
  ...
  if(path)
  {
    ...
    if(dllVer)
    {
      ...
      {
        report = report_fixed_ver(installed_version:dllVer, fixed_version:"14.0.7141.500015.0.4709.1000", install_path:path);
        security_message(data:report);
        exit(0);
      }
    }
